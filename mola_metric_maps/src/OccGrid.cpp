...
void OccGrid::setSize(
    ...
    float resolution, float occupancyValue)
{
  grid_.setSize(minCorner.x, maxCorner.x, minCorner.y, maxCorner.y, resolution, occupancyValue);
...
void OccGrid::resizeGrid(
    const mrpt::math::TPoint2Df& minCorner, const mrpt::math::TPoint2Df& maxCorner,
    float newCellsOccupancy) noexcept
{
  grid_.resizeGrid(minCorner.x, maxCorner.x, minCorner.y, maxCorner.y, newCellsOccupancy);

  const auto defValue = CACHE_MISS_VALUE;
  likelihoodCacheGrid_.resize(
      grid_.getXMin(), grid_.getXMax(), grid_.getYMin(), grid_.getYMax(), defValue,
      0.5 /* extra margin [meters] */);
}
...
void OccGrid::resetLikelihoodCache()
{
  ...
  likelihoodCacheGrid_.setSize(
      ...
      grid_.getResolution() * likelihoodParameters_.superResolutionFactor, &defValue);
}
