...
bool eval () //string result_sha,Mail* mail)
{
  ...
  string gt_dir         = "/home/jlblanco/datasets-slam/kitti-odometry-slam/gt-poses";
  string result_dir     = "/home/jlblanco/code/mola/demos/results/";
  ...
  //for (int32_t i=11; i<22; i++) {
    ...
    // plot status
    printf("Processing: %s, poses: %d/%d\n",file_name,poses_result.size(),poses_gt.size());

    printf("-- poses_gt: %u entries\n", poses_gt.size());
    printf("-- poses_result: %u entries\n", poses_result.size());
