...
void RawlogDataset::initialize_rds(const Yaml& c)
{
  ...
  const auto imgsDir = mrpt::obs::CRawlog::detectImagesDirectory(rawlog_filename_);
  if (mrpt::system::directoryExists(imgsDir))
  {
    mrpt::io::setLazyLoadPathBase(imgsDir);
    MRPT_LOG_INFO_STREAM("Setting rawlog external directory to: " << imgsDir);
  }
  ...
  {
    if (!rawlog_in_.open(rawlog_filename_)) throw std::runtime_error("Cannot open input rawlog!");
  }
...
void RawlogDataset::spinOnce()
{
  ...
  if (rawlog_begin_time_ == INVALID_TIMESTAMP) rawlog_begin_time_ = read_ahead_.begin()->first;
  ...
  if (read_all_first_ && teleport_here.has_value() && *teleport_here < rawlog_entire_.size())
  {
    rawlog_next_idx_ = *teleport_here;
    if (auto obs = rawlog_entire_.getAsObservation(rawlog_next_idx_); obs)
      last_dataset_time_ = mrpt::system::timeDifference(rawlog_begin_time_, obs->timestamp);
  }
  else
  {
    if (paused)
  ...
  while (!read_ahead_.empty() &&
         last_dataset_time_ >= timeDifference(rawlog_begin_time_, read_ahead_.begin()->first))
  {
    //
    ...
    MRPT_LOG_DEBUG_STREAM(
        "Publishing " << obs->GetRuntimeClass()->className << " sensorLabel: " << obs->sensorLabel
                      << " for t=" << last_dataset_time_ << " observation timestamp="
  ...
  while (read_ahead_.size() < READ_AHEAD_LEN)
  {
    try
    {
      ...
      {  // Single observation:
        ...
        if (auto sf = std::dynamic_pointer_cast<mrpt::obs::CSensoryFrame>(obj); sf)
        {
          for (const auto& o : *sf) read_ahead_.emplace(o->getTimeStamp(), o);
        }
        else if (auto acts = std::dynamic_pointer_cast<mrpt::obs::CActionCollection>(obj); acts)
        {
...
{
  while (read_ahead_.size() < READ_AHEAD_LEN && rawlog_next_idx_ < rawlog_entire_.size())
  {
...
mrpt::obs::CSensoryFrame::Ptr RawlogDataset::datasetGetObservations(size_t timestep) const
{
  ASSERTMSG_(
      read_all_first_,
      "Using the OfflineDatasetSource API in this class requires setting "
