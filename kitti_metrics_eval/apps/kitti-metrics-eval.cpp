...
static void do_kitti_eval_error(Cli& cli)
{
  ...
  if (const char* s = ::getenv("KITTI_BASE_DIR"); s != nullptr) cli.kitti_basedir = s;
...
std::vector<Matrix> loadPoses_tum_format(
    Cli& cli, const std::string& file_name, const std::string& calib_file, bool isGT)
{
  mrpt::poses::CPose3DInterpolator trajectory;

  if (!trajectory.loadFromTextFile_TUM(file_name))
    throw std::runtime_error(mrpt::format("Error loading trajectory from: %s", file_name.c_str()));
...
std::vector<Matrix> loadPoses(string file_name)
{
  ...
  while (!feof(fp))
  {
    ...
    if (fscanf(
            fp, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf", &P(0, 0), &P(0, 1), &P(0, 2),
            &P(0, 3), &P(1, 0), &P(1, 1), &P(1, 2), &P(1, 3), &P(2, 0), &P(2, 1), &P(2, 2),
            &P(2, 3)) == 12)
      poses.push_back(P);
  }
  fclose(fp);
...
vector<errors> calcSequenceErrors(vector<Matrix>& poses_gt, vector<Matrix>& poses_result)
{
  ...
  for (int32_t first_frame = 0; first_frame < poses_gt.size(); first_frame += step_size)
  {
    ...
    for (int32_t i = 0; i < num_lengths; i++)
    {
      ...
      Matrix pose_delta_result = (poses_result[first_frame]).inverse() * poses_result[last_frame];
...
void saveSequenceErrors(vector<errors>& err, string file_name)
{
  ...
  ASSERTMSG_(fp, mrpt::format("Could not open for writing: %s", file_name.c_str()));
...
void savePathPlot(vector<Matrix>& poses_gt, vector<Matrix>& poses_result, string file_name)
{
  ...
  for (int32_t i = 0; i < poses_gt.size(); i += step_size)
    fprintf(
        fp, "%f %f %f %f\n", poses_gt[i](0, 3), poses_gt[i](2, 3), poses_result[i](0, 3),
        poses_result[i](2, 3));
...
void plotPathPlot(Cli& cli, string dir, vector<int32_t>& roi, int32_t idx)
{
  ...
  if (!cli.argSkipFigures.isSet())
  {
    ...
    system(command);
    sprintf(
        command, "bash -c 'cd %s; pdfcrop %02d_large.pdf %02d.pdf > /dev/null 2>&1'", dir.c_str(),
        idx, idx);
    system(command);
...
void saveStats(vector<errors> err, string dir)
{
  ...
  printf(
      ...
      "rot_error(deg/100m)\n");
  printf("%f  %f %f\n", 100 * t_err / num, r_err / num, (180.0 / M_PI) * 100 * r_err / num);
...
bool eval(Cli& cli)  // string result_sha,Mail* mail)
{
  ...
  if (!mrpt::system::directoryExists(error_dir)) mrpt::system::createDirectory(error_dir);
  if (!mrpt::system::directoryExists(plot_path_dir)) mrpt::system::createDirectory(plot_path_dir);
  if (!mrpt::system::directoryExists(plot_error_dir)) mrpt::system::createDirectory(plot_error_dir);
  ...
  for (const auto& seq : seqs)
  {
    ...
    else
    {
      if (seq.is_kitti)
      {
        ...
        poses_gt     = loadPoses(seq.kitti_gt_poses_file);
        poses_result = loadPoses_tum_format(cli, seq.result_file, seq.kitti_calib_file, false);
      }
      else
      {
        poses_gt     = loadPoses_tum_format(cli, seq.custom_gt_tum_file, {}, false);
        poses_result = loadPoses_tum_format(cli, seq.result_file, {}, false);
      }
    }

    // plot status
    printf(
        "Processing: %s, poses: %ld/%ld\n", seq.file_name.c_str(), poses_result.size(),
        poses_gt.size());
